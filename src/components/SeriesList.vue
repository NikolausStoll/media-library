...
async function searchTmdbSeries() {
  ...
  try {
    const results = await searchTmdb(q, 'series')
    const existingIds = new Set(seriesList.value.map(s => String(s.externalId)))
...
<template>
  <div
    :class="['app-layout', 'theme-series', { 'light-mode': !darkMode }]"
