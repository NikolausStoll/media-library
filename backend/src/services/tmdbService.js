...
export async function getMovie(id) {
  const [resDe, resEn] = await Promise.all([
    fetch(buildUrl(`/movie/${id}`, { append_to_response: 'release_dates', language: 'de-DE' })),
    fetch(buildUrl(`/movie/${id}`, { language: 'en-US' }))
  ])
  ...
  return {
    ...
    seasons:      null,
    episodes:     null,
    genres:       JSON.stringify(de.genres?.map(g => g.name) ?? []),
...
export async function getSeries(id) {
  const [resDe, resEn] = await Promise.all([
    fetch(buildUrl(`/tv/${id}`, { append_to_response: 'content_ratings', language: 'de-DE' })),
    fetch(buildUrl(`/tv/${id}`, { language: 'en-US' }))
  ])
