...
export function saveToCache(item) {
  db.prepare(`
    INSERT INTO tmdbcache
      (id, mediaType, titleEn, titleDe, imageUrl, year, certification, rating, runtime, seasons, episodes, genres, linkUrl, originalLang, updatedAt)
    VALUES
      (@id, @mediaType, @titleEn, @titleDe, @imageUrl, @year, @certification, @rating, @runtime, @seasons, @episodes, @genres, @linkUrl, @originalLang, @updatedAt)
    ON CONFLICT(id, mediaType) DO UPDATE SET
      ...
      genres=excluded.genres, linkUrl=excluded.linkUrl, originalLang=excluded.originalLang,
